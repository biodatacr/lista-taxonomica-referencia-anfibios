---
title: "Lista taxonómica de referencia de anfibios de Costa Rica"
...
CHECKLIST <- "data/processed/lista-taxonomica-referencia-anfibios.csv"
OCCURRENCES_CHECKLIST <- "data/processed/registros-presencia-lista-taxonomica-referencia-anfibios-gbif.csv"
OCCURRENCES_AMPHIBIANS <- "data/processed/registros-presencia-anfibios-gbif.csv"
```
...
[lista taxonómica de referencia de anfibios de Costa Rica](data/processed/lista-taxonomica-referencia-anfibios.csv)

El repositorio con los datos y el código fuente de los programas está en la dirección:  
[https://github.com/biodatacr/lista-taxonomica-referencia-anfibios](https://github.com/biodatacr/lista-taxonomica-referencia-anfibios)
...
La verificación de los nombres científicos en la lista de GBIF se llevó a cabo mediante un programa en el lenguaje [R](https://www.r-project.org/) y su paquete [rgbif](https://cran.r-project.org/web/packages/rgbif/), el cual proporciona acceso a los servicios web que componen la [interfaz de programación de aplicaciones (API) de GBIF](https://www.gbif.org/developer/summary). Como resultado de esta verificación, se detectaron errores ortográficos, sinónimos, y se obtuvieron elementos adicionales de datos, como autores y niveles taxonómicos superiores, entre otros.

Seguidamente, se describen en detalle las etapas del proceso de elaboración de la lista.

### 1. Lista inicial
La [lista inicial](data/raw/Reptiles_Anfibios2022.xlsx) contiene 216 especies de anfibios de Costa Rica y fue recopilada en el contexto del proyecto Biodiversidad en cifras [@bermudez_rojas_tania_biodiversidad_2021]. El documento fue facilitado en formato XLSX (Microsoft Excel).

### 2. Lista intermedia
Se removieron de la lista inicial aquellas filas que no corresponden a datos (ej. título, autor, fecha, etc.) y el resultado se guardó en formato CSV, como una [lista intermedia](data/interim/anfibios_2022.csv). De esta manera, se obtuvo un conjunto de datos debidamente estructurado, con una fila por cada especie y una columna por cada elemento de datos.
...
La [lista final](data/processed/lista-taxonomica-referencia-anfibios.csv) consta de los 216 nombres de especies de la lista original, revisados y corregidos, junto con los restantes elementos de datos recuperados de la lista de referencia taxonómica de GBIF.
...
Para tener un idea de los datos de presencia disponibles para las especies de la lista, se realizó una [consulta](https://https://doi.org/10.15468/dl.rn97yg) al Portal de datos de GBIF. Se encontraron `r nrow(occurrences_checklist)` registros de `r occurrences_checklist |> st_drop_geometry() |> select(species) |> n_distinct()` especies.
...
Se realizó otra [consulta](https://doi.org/10.15468/dl.wndm84) al Portal de datos de GBIF, de todos los registros de presencia de la clase *Amphibia* (todas las especies, no solo las de la lista de referencia), la cual retornó `r nrow(occurrences_amphibians)` registros de `r occurrences_amphibians |> st_drop_geometry() |> select(species) |> n_distinct()` especies.
...
occurrences_amphibians_groupedby_species <-
  occurrences_amphibians |>
  st_drop_geometry() |>
  filter(species != "") |>
  group_by(order, family, genus, species) |>
